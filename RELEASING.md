...
`github:mat3ra/standata#branch` (which has no way to build `dist/` on install), CI publishes
...
```text
https://github.com/mat3ra/standata/releases/download/wip-<short-sha>/standata.tgz
```
...
```bash
npm run mat3ra:install -- standata wip-<short-sha>
```

(`web-app/scripts/mat3ra-install.sh` — installs `@mat3ra/standata@<release-url>` with
